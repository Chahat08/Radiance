...
class UserPostListView(LoginRequiredMixin, ListView):
	model = Post
	template_name = 'tasks/user_posts.html'
	context_object_name = 'posts'
	paginate_by = 10
...
def post_detail(request, pk):
	...
	if request.method == 'POST':
		...
		form = NewCommentForm()
	return render(request, 'tasks/post_detail.html', {'post':post, 'is_liked':is_liked, 'form':form})
...
def create_post(request):
	...
	if request.method == "POST":
		...
		form = NewPostForm()
	return render(request, 'tasks/create_post.html', {'form':form})

class PostUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
	model = Post
	fields = ['description', 'pic', 'tags']
	template_name = 'tasks/create_post.html'
...
def search_posts(request):
	...
	return render(request, "tasks/search_posts.html", context)
